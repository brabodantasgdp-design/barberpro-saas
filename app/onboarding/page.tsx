...
export default async function Onboarding({searchParams}:{searchParams:Promise<{error?:string}>}){
 ...
 const {data:membership}=user?await supabase.from("shop_members").select("id,shop_id").eq("user_id",user.id).limit(1).maybeSingle():{data:null};
 if(membership)return <main className="onboardingPage"><SetupWizard shopId={membership.shop_id} staffId={membership.id}/></main>;
