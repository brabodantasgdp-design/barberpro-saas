 ...
 return <>
  ...
  <div className="grid">
   <section className="card"><div className="panelhead"><b>Próximos atendimentos</b><span className="muted">Hoje</span></div><div className="panelbody staff">
    {valid.length===0?<EmptyState eyebrow="Sua agenda está livre" title="Nenhum atendimento hoje" description="Compartilhe seu link público ou crie um agendamento manual para começar a movimentar o dia." action="Abrir agenda" href="/agenda"/>:valid.slice(0,8).map(a=><div className="staffrow" key={a.id}>
   ...
   <section className="card"><div className="panelhead"><b>Produção da equipe</b></div><div className="panelbody staff">
    {staff.length===0?<EmptyState eyebrow="Time da casa" title="Monte sua equipe" description="Adicione profissionais para acompanhar produção, comissão e disponibilidade." action="Adicionar equipe" href="/equipe"/>:staff.map(p=>{const own=valid.filter(a=>(a as any).staff_id===p.id);const gross=own.reduce((s,a)=>s+a.price_cents,0);return <div className="staffrow" key={p.id}>
